...
def _softHeader(payload):
    ...
    if isinstance(payload, str) and payload.startswith("shz1"):
        t = payload.find("a0a0")
        if t >= 0:
            header = payload[:t + 4]
            body = payload[t + 4:]
            ...
            while ok and i < len(core):
                j = i
                while j < len(core) and 48 <= ord(core[j]) <= 57:
                    j += 1
                ok = j > i
                if ok:
                    nums.append(int(core[i:j]))
                if ok and j < len(core):
                    ok = core[j] == _sepChar(sepIdx)
                    if ok:
                        sepIdx += 1
                        i = j + 1
                elif ok:
            ...
            if ok and len(nums) >= 4:
                ...
                if total > 0:
                    out["lens"] = [len(x) for x in _splitEven(body, total)]

                if total == len(lens) and sum(lens) == len(body):
                    out["lens"] = lens
                    out["hasHeader"] = True
...
# Build Version: 45A
# NOTES: Integer/text conversion, custom-base transforms, reversible digit manipulation, rotation, splitting, and packing helpers.
# =========================
def toBytes(t):
...
# =========================
# Key schedule and core transform pipeline
# Build Version: 45A
# NOTES: Key evolution, effective-base extraction, and forward/reverse layered transforms used by both encryption and decryption.
# =========================
...
def decryptData(n, k):
    ...
    payloadGuess = deobfuscate(n, hKey)
    metaGuess = _softHeader(payloadGuess)

    totalSteps = len(metaGuess["lens"]) + 3
    done = 0

    payload = _deobfuscateProg(n, hKey, 64, "DEC", done, totalSteps)
    done += 3
    ...
    for L in lens:
        ...
        pos += L
        pInt = _decryptIntWithKey(cPart, hKey)
