...
# Lottery Key 333 Bit
# Build Version: 51A
# NOTES: Produces a deterministic 256 bit key peppered with 8 base46 characters distributed deterministically
# =========================
...
def lotKey(n, count=8):
    ...
    if not isinstance(count, int): raise TypeError("count must be int")
    if count < 1 or count > 8: raise ValueError("count must be in 1..8")

    aux = "ghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    ...
    def mix(tag, *parts):
        return fold64(pack(tag, *parts))

    def derive(state, tag, *parts):
        a = mix(tag, "A", state, *parts)
        b = mix(tag, "B", a[::-1], state, *parts)
        c = mix(tag, "C", b[::-1], a, state, *parts)
        return mix(tag, "D", a, b, c, state, *parts)

    def stream(state, tag):
        ctr = 0
        cur = state
        while True:
            a = mix(tag, "X", cur, ctr)
            b = mix(tag, "Y", a[::-1], cur, ctr)
            c = derive(cur, tag, ctr, a, b)
            cur = mix(tag, "Z", cur, a, b, c, ctr)
            ...
            while i < 64:
                ...
                cc = int(c[i:i+2], 16)
                yield (aa ^ bb) & 255
                yield (bb + cc + (i >> 1) + ctr) & 255
                yield (cc ^ aa ^ ((ctr * 17 + i) & 255)) & 255
                i += 2
            ctr += 1

    def pickVals(gen, need):
        out = []
        while len(out) < need:
            b = next(gen)
            if b < 230:
    ...
    def makeBody(gen, need):
        out = []
        while len(out) < need:
            b = next(gen)
            out.append(hexs[b >> 4])
            if len(out) < need:
                out.append(hexs[b & 15])
        return "".join(out)

    def permute(text, gen):
        arr = list(text)
        i = len(arr) - 1
        while i > 0:
            size = i + 1
    ...
    seedHex = generatePKey(n).lower()
    if not isinstance(seedHex, str) or len(seedHex) != 64:
        raise ValueError("generatePKey must return 64 chars")

    seedInt = tDecimal(seedHex, 16)
    deepA = checkData(seedInt + n + count)
    deepB = checkData(toBytes(pack("LOTROOT", n, count, seedHex, deepA)))

    root = mix("LOTROOT", n, count, seedHex, seedInt, deepA, deepB)
    posState = derive(root, "LOTPOS", n, count, seedHex, deepA)
    valState = derive(root, "LOTVAL", n, count, seedHex, deepB)
    bodyState = derive(root, "LOTBODY", n, count, seedHex, deepA, deepB)
    permState = derive(root, "LOTPERM", n, count, seedHex, deepA ^ deepB if isinstance(deepA, int) and isinstance(deepB, int) else str(deepA) + str(deepB))

    posGen = stream(posState, "LOTPOS")
    valGen = stream(valState, "LOTVAL")

    vals = pickVals(valGen, count)
    pos = pickPos(posGen, totalLen, count)

    pairs = []
    i = 0
    while i < count:
        pairs.append((pos[i], aux[vals[i]], vals[i]))
        i += 1
    pairs.sort()
    ...
    i = 0
    while i < len(pairs):
        sched.append(str(pairs[i][0]).zfill(2))
        sched.append(pairs[i][1])
        sched.append(str(pairs[i][2]).zfill(2))
        i += 1
    sched = "".join(sched)

    bodyState = derive(bodyState, "LOTBODY", root, seedHex, sched, n, count)
    permState = derive(permState, "LOTPERM", bodyState, sched, root)

    bodyGen = stream(bodyState, "LOTBODY")
    permGen = stream(permState, "LOTPERM")

    body = makeBody(bodyGen, 64)
    body = permute(body, permGen)
